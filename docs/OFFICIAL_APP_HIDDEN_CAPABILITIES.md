...
6. **Account management surface — completed**: the same module and the GUI's **Manage accounts** tab reproduce the native player operations `RemoveAccount`, `EditAccountPasswordX`, `EditAccountMd`, `RefreshAccountCredentialsX`, `SetAccountNicknameX`, and `GetWebCode` with the exact fields from the decompiled builders (`FUN_100e60cb0`, `FUN_100e60eb0`, `FUN_100e61520`, `FUN_100e612d0`, `FUN_100e6...`, `FUN_100e60530`). `RemoveAccount` takes the account UDN as the native `AccountID`; `EditAccountPasswordX`/`EditAccountMd` take the account key (`X_#Svc…-Token`, stored as Username0) — verified live that the full UDN is rejected for edits with UPnP error 806 while the key succeeds; `RefreshAccountCredentialsX` takes the numeric `AccountUID` embedded in that UDN. All mutations re-verify the target household and require explicit confirmation.

   Live-household findings refined this surface. `local_soap` now surfaces the player's numeric UPnP error code (parsed from the fault `<detail>`), so rejections read `(UPnP error 806: account could not be resolved)` instead of the opaque `UPnPError`. Local `SetAccountNicknameX` **works** and renames any account: both `AccountUDN` and the nickname must be wrapped in the household `2:` envelope (AES-128-CBC under the household-derived key, the same envelope as `ThirdPartyMediaServersX`). Sending plaintext values is rejected with UPnP error 402 — that was the cause of every earlier rename failure, not the firmware. The envelope was cracked from a wire capture of the Windows controller (which was observed renaming SiriusXM locally, targeting `SA_RINCON9479_X_#Svc9479-1f8c0a56-Token` with nickname `martinroff`, both `2:`-encoded) and verified live on a household player: the GUI enables Set nickname for every record — including keyless anonymous records, which the player accepts renames for (verified live), even though the official app hides the action for them. Anonymous services commit with an empty account ID: the player rejects any other value for anonymous `AddAccountX` (402), producing keyless records (no provider key or username in the UDN) that remain browsable. Such records are removed again with the empty-key `RemoveAccount` contract — verified live: `RemoveAccount(type, "")` returns 200 and removes exactly that service's keyless record, and the GUI's **Manage accounts** tab keeps Remove enabled for them. (An earlier 806 observation for an empty key came from probing an account that had already been removed; it proved nothing about keyless records.) The legacy `GetWebCode` returns UPnP error 800 for every service, so the GUI no longer offers it; the module keeps the faithful action and translates the rejection.
